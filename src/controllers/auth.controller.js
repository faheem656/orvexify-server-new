/** DROP-IN: src/controllers/auth.controller.js
 *
 * Same APIs as the old authRoutes.js — logic yahan, routes thin file mein.
 * Login (password / 2FA / backup code) pe lastLoginAt + lastLoginIP.
 *
 * Do NOT remount. Keep existing:
...
async function stampLogin(user, req) {
  try {
    if (typeof user.updateLastLogin === 'function') {
      await user.updateLastLogin(clientIp(req));
      return;
    }
    ...
    console.error('lastLogin stamp failed:', err.message);
  }
}
...
const generateBookingSlug = async (clinicName) => {
  let baseSlug = String(clinicName || '')
    .toLowerCase()
    .trim()
    .replace(/[^\w\s-]/g, '')
    .replace(/\s+/g, '-')
      ...
      if (backupCode) {
        ...
        const html = `
            <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto;">
              <div style="background: linear-gradient(135deg, #803AFF, #4E49FF); padding: 30px; text-align: center;">
        ...
        return res.json({
          ...
          token,
          user: publicUser(user, { isTwoFactorEnabled: false }),
        });
      }
...
async function getSession(req, res) {
  res.json({
    ...
    user: publicUser(req.user, {
      bookingSlug: req.user.bookingSlug,
    }),
    ...
    const html = `
      ...
      <head>
        ...
        <style>
          ...
          .header { background: linear-gradient(135deg, #803AFF, #4E49FF); padding: 30px 20px; text-align: center; }
    ...
    const html = `
      ...
      <head>
        ...
        <style>
          ...
          .header { background: linear-gradient(135deg, #803AFF, #4E49FF); padding: 30px; text-align: center; border-radius: 10px 10px 0 0; }
          .header h1 { color: white; margin: 0; }
          .content { background: #f8fafc; padding: 30px; border-radius: 0 0 10px 10px; }
          .button { display: inline-block; padding: 12px 24px; background: #4E49FF; color: white; text-decoration: none; border-radius: 8px; }
          .footer { text-align: center; padding: 20px; color: #94a3b8; font-size: 12px; }
        </style>
      </head>
      <body>
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto;">
        ...
        <div style="padding: 30px; background: #f8fafc;">
          ...
          <div style="background: white; padding: 20px; text-align: center; font-size: 32px; font-weight: bold; letter-spacing: 8px; color: #4E49FF;">${otp}</div>
