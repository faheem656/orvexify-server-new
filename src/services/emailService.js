...
// ============ GENERATE TRACKING HTML (CSS BACKGROUND METHOD) ============
const generateTrackingHTML = (pixelUrl, trackingToken) => {
  return `
    <!-- ============ CSS BACKGROUND TRACKING (MOST RELIABLE) ============ -->
    
    <!-- 1. Inline CSS Background - Primary Method -->
    <div style="
      background-image: url('${pixelUrl}?method=css&t=${Date.now()}');
    ...
    <!-- 2. Multiple Background Images - Redundancy -->
    <div style="
      background-image: 
        url('${pixelUrl}?method=css1&t=${Date.now()}'),
        url('${pixelUrl}?method=css2&t=${Date.now()+1}'),
        url('${pixelUrl}?method=css3&t=${Date.now()+2}');
      background-size: 0 0;
      width: 0px;
      height: 0px;
      display: block !important;
    ...
    <!-- 3. CSS @import - Advanced Method -->
    <style>
      @import url('${pixelUrl}?method=import&t=${Date.now()}');
      ...
      .tracking-${trackingToken}::before {
        content: '';
        background-image: url('${pixelUrl}?method=pseudo&t=${Date.now()}');
        background-size: 0 0;
        display: block;
        width: 0px;
        height: 0px;
    ...
    <!-- 4. Table Background - Outlook Support -->
    <table style="display:none !important; width:0; height:0; mso-hide:all; border:0; cellpadding:0; cellspacing:0;">
      <tr>
        <td style="
          background-image: url('${pixelUrl}?method=table&t=${Date.now()}');
          background-size: 0 0;
          width: 0px;
          height: 0px;
          font-size: 0;
    ...
    <!-- 5. List Item Background -->
    <ul style="display:none !important; margin:0; padding:0; mso-hide:all;">
      <li style="
        background-image: url('${pixelUrl}?method=li&t=${Date.now()}');
        background-size: 0 0;
        list-style: none;
        width: 0px;
        height: 0px;
    ...
    <!-- 6. Standard Image Pixel - Fallback -->
    <img src="${pixelUrl}?method=img&t=${Date.now()}" 
         alt="" 
         width="1" 
         height="1" 
         style="display:block !important; width:1px !important; height:1px !important; max-width:1px !important; max-height:1px !important; overflow:hidden !important; opacity:0.001 !important; position:absolute !important; pointer-events:none !important;" 
         border="0" />
    
    <!-- 7. IFrame - Last Resort -->
    <iframe src="${pixelUrl}?method=iframe&t=${Date.now()}" 
            width="0" 
            height="0" 
            style="display:none !important; border:0;"></iframe>
  `;
};
...
const sendBookingConfirmation = async (
  ...
  if (!to) {
    console.error("❌ sendBookingConfirmation: No recipient");
    return { success: false, error: "No recipient email" };
  }
  to = String(to).trim();
  ...
  const html = `
    ...
    <head>
      ...
      <style>
        ...
        .header { 
          background: ${urgencyStyles.headerBg}; 
          padding: 32px 24px; 
          text-align: center; 
        }
        .header h1 { 
        ...
        .urgency-badge {
          ...
          color: ${urgencyStyles.badgeColor};
          padding: 6px 16px;
          border-radius: 50px;
          font-size: 13px;
          font-weight: 700;
        ...
        .warning-box {
          ...
          padding: 14px 18px;
          margin: 16px 0;
          display: ${urgencyStyles.showWarning ? 'block' : 'none'};
        }
        .warning-box p {
          margin: 0;
          color: #991b1b;
        ...
        .details { 
          ...
          border-left: 4px solid ${urgencyStyles.borderColor};
        }
        .details p {
          margin: 8px 0;
          font-size: 15px;
        ...
        .btn-confirm {
          background: ${urgencyLevel === 'high' ? '#dc2626' : '#22c55e'};
          color: white;
        }
        .btn-confirm:hover {
          background: ${urgencyLevel === 'high' ? '#b91c1c' : '#16a34a'};
      ...
      <div class="container">
        ...
        <div class="content">
          ...
          <div style="background: #fef2f2; border-radius: 8px; padding: 12px 16px; margin: 12px 0;">
            <p style="margin: 0; font-size: 13px; color: #991b1b; text-align: center;">
              ⚠️ Please respond immediately. Your appointment is in less than 30 minutes.
            </p>
          </div>
          ` : ''}
          ...
          <div style="background: #fffbeb; border-radius: 8px; padding: 10px 14px; margin: 12px 0;">
            <p style="margin: 0; font-size: 13px; color: #92400e; text-align: center;">
              🔔 Please confirm or cancel within the next 2 hours.
            </p>
          </div>
          ` : ''}
          
          <p style="font-size: 13px; color: #64748b; text-align: center; margin: 12px 0 0;">
            Please confirm or cancel at least 2 hours before your appointment.
          </p>
        ...
        <div class="footer">
          <p style="margin: 0;">
            ...
            &nbsp;|&nbsp;
            <a href="${baseUrl}/unsubscribe/${trackingToken}">Unsubscribe</a>
          </p>
...
const sendCancellationEmail = async (
  ...
  if (!to) {
    console.error("❌ sendCancellationEmail: No recipient");
    return { success: false, error: "No recipient email" };
  }
  to = String(to).trim();
  if (!to) return { success: false, error: "Invalid email" };
...
const sendConfirmedEmail = async (
  ...
  if (!to) {
    console.error("❌ sendConfirmedEmail: No recipient");
    return { success: false, error: "No recipient email" };
  }
  to = String(to).trim();
  if (!to) return { success: false, error: "Invalid email" };
...
const sendNoResponseFollowUp = async (
  ...
  if (!to) {
    console.error("❌ sendNoResponseFollowUp: No recipient");
    return { success: false, error: "No recipient email" };
  }
  to = String(to).trim();
  if (!to) return { success: false, error: "Invalid email" };
  ...
  const html = `
    ...
    <head>
      <meta charset="UTF-8">
      <title>Appointment Reminder - Follow Up</title>
      <style>
        body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; }
        .container { max-width: 500px; margin: 0 auto; background: #ffffff; }
        ...
        .content { padding: 30px; background: #f8fafc; border-radius: 0 0 10px 10px; }
        .details { background: white; padding: 20px; border-radius: 10px; margin: 20px 0; border: 1px solid #e2e8f0; }
        .btn-confirm { display: inline-block; padding: 12px 24px; background: #22c55e; color: white; text-decoration: none; border-radius: 8px; margin: 5px; font-weight: 600; }
        .btn-cancel { display: inline-block; padding: 12px 24px; background: #ef4444; color: white; text-decoration: none; border-radius: 8px; margin: 5px; font-weight: 600; }
        .footer { text-align: center; padding: 20px; color: #94a3b8; font-size: 12px; }
